  ...
  render() {
    ...
    return (
      <div>
        ...
        <div>{this.getInput(this.modifier)}</div>
      </div>
    )
  }
})
...
export const SelectItem = Vue.extend({
  ...
  render() {
    ...
    return (
      <div>
        <vautocomplete {...data}></vautocomplete>
      </div>
    )
  }
})
...
export const Filter = Vue.extend({
  ...
  methods: {
    ...
    filterButton(op: FilterItem, on: any, isFoldedItem: boolean) {
      ...
      const body = (
        <span>
          <vicon left={true} on={{ click: (e: any) => this.clear(e, op) }}>
          ...
          <span>
            ...
            {op.selected ? (
              <span>
                {' '}
                | <span class={'primary--text'}>{showValue}</span>
              </span>
            ) : null}
          </span>
        </span>
      ...
      if (isFoldedItem) {
        return (
          <vlistitem on={on} dense={true} class={'my-1 px-2'}>
            {body}
          </vlistitem>
        )
      }
  ...
  render() {
    ...
    const otherSelectedFilters = this.filtersGetFunc(
      (item) => item.folded && !!item.selected,
      false
    )(itemTypes, trans)
    const foldedFilters = this.filtersGetFunc((item) => item.folded && !item.selected, true)(
      itemTypes,
      trans
    )
        ...
        {foldedFilters.length > 0 ? (
          <vmenu
            scopedSlots={{
              activator: ({ on }: any) => {
                return (
                  <vbtn on={on} plain={true} small={true} color={'primary'} class={'my-1'}>
            ...
            rounded={'md'}
          >
            <vlist flat={true} class={'white pa-0'}>
              {foldedFilters}
            </vlist>
          </vmenu>
        ) : null}

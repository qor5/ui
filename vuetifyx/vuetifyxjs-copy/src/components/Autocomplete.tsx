...
export default Vue.extend({
  ...
  methods: {
    loadRemoteItems() {
      ...
      ;(this as any)
        .$plaid()
        .url(this.remoteUrl)
        .eventFunc(this.eventName)
        .query('keyword', this.searchKeyword)
        .query('page', this.remote.page)
        .go()
        .then((r: any) => {
  ...
  render(h: CreateElement): VNode {
    const { remoteUrl, hideDetails } = this.$props

    let { hideSelected, cacheItems } = this.$props
    ...
    if (remoteUrl) {
      ...
      if (this.isPaging) {
        ...
        slots.push(
          <template slot="append-item">
            <div class="text-center">
              <v-btn {...loadmoreNodeData}>Load more</v-btn> <v-divider vertical></v-divider>{' '}
              <span>
                {this.remote.current}/{this.remote.total}
              </span>
            </div>
          </template>
        )
    ...
    return (
      <div>
          ...
          <v-card>
            <v-list>
              <vx-draggable
                animation="300"
                handle=".handle"
                value={this.cachedSelectedItems}
                onInput={this.changeOrder}
              >
                {this.cachedSelectedItems.map((item: any, i) => {
                  return (
                    <div key={item.value}>
                      ...
                      {i < this.cachedSelectedItems.length - 1 ? (
        ...
        <vautocomplete {...autocompleteData}>{slots}</vautocomplete>
      </div>
    )
  }
})
